--[[
    XDEMIC CHAT v15.2 | LocalScript -> StarterPlayerScripts
    Full Feature Pack + Follow System + Leaderboard by Followers
--]]
...
--  FOLLOW SYSTEM
-- ============================================================
local FollowCache = {}  -- cache for followers count per userId
...
-- ============================================================
--  LEADERBOARD (sorted by followers count)
-- ============================================================
...
-- ============================================================
--  QUICK ACTIONS & CONTEXT MENU (enhanced with Follow)
-- ============================================================
local QuickActionMenu = make("Frame",{
    Size=UDim2.new(0,140,0,100),  -- increased height for follow button
    BackgroundColor3=C.BG2,
    Visible=false,
    ZIndex=990,
    Parent=Gui
...
-- ============================================================
--  PROFILE PAGE (enhanced with followers/following)
-- ============================================================
...
local followersLabel = make("TextLabel",{Size=UDim2.new(1,-12,0,18),Position=UDim2.new(0,6,0,130),BackgroundTransparency=1,Text="👥 Followers: 0  |  Following: 0",TextColor3=C.GRAY,TextSize=11,Font=Enum.Font.Gotham,ZIndex=502,Parent=ProfPage})

-- Update counts
local function updateProfileCounts()
    getFollowersCount(tostring(Player.UserId), function(count)
        followersLabel.Text = "👥 Followers: "..count.."  |  Following: ..."
        -- Also get following count
        local response = makeHttpRequest({
...
local DIn=mkBox("Change Display name...",160) DIn.Text=ClientProfileData.DisplayName
local BIn=mkBox("Write profile bio description...",198,54) BIn.MultiLine=true
local RankIn=mkBox("Custom Rank Tag (e.g. VIP, Mod)",256,34) RankIn.Text=ClientProfileData.RankTag
local ColorIn=mkBox("Rank Color (hex, e.g. #FF5733)",294,34) ColorIn.Text=ClientProfileData.RankColor
local AvatarIn=mkBox("Avatar URL (Roblox ID or image link)",332,34) AvatarIn.Text=ClientProfileData.AvatarUrl
local FontIn=mkBox("Font (Gotham, Arial, ComicSans, etc.)",370,34) FontIn.Text=ClientProfileData.Font

local SvB=make("TextButton",{Size=UDim2.new(1,-14,0,38),Position=UDim2.new(0,7,0,414),BackgroundColor3=C.BLUE,Text="APPLY CHANGES & SYNC",TextColor3=C.WHITE,TextSize=12,Font=Enum.Font.GothamBold,ZIndex=502,Parent=ProfPage})
corner(SvB,10)
...
-- ============================================================
--  LEADERBOARD PAGE (sorted by followers)
-- ============================================================
...
local function refreshLeaderboard()
    ...
    -- Fetch all followers data to count per user
    local response = makeHttpRequest({
    ...
    -- For each user, count followers
    local userCounts = {}
    for userId, _ in pairs(followersData) do
        -- fetch the actual list of followers for this userId (shallow to get count)
