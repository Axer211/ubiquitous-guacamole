--[[
    ...
    MODIFIED: Messages now stay forever; cooldown added: text 2s, sticker 10s
    FIXED: Cooldown now uses tick() for real-time accuracy
--]]
...
-- Cooldown timers (using tick() for real-time accuracy)
local lastTextSendTime = 0
local lastStickerSendTime = 0
...
-- ============================================================
--  CHAT MESSAGE ADDERS (with auto‑removal removed)
-- ============================================================
...
local function addMsg(sender, text, isImg, imgUrl, overrideDisplayName)
    ...
    -- REMOVED: auto-removal of messages (they stay forever)
...
-- ============================================================
--  STICKER SEND HELPER WITH COOLDOWN (using tick() for real-time)
-- ============================================================
local function sendSticker(url)
    if url == "" then return end
    -- Cooldown: 10 seconds between stickers
    if tick() - lastStickerSendTime < 10 then
        addSys("⏳ Please wait 10 seconds before sending another sticker.")
        return
    end
    lastStickerSendTime = tick()
...
local function sendMsg()
    ...
    -- Normal message with 2-second cooldown (using tick() for real-time)
    if tick() - lastTextSendTime < 2 then
        addSys("⏳ Please wait 2 seconds before sending another message.")
        return
    end
    lastTextSendTime = tick()
