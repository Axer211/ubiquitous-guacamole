--[[
    XDEMIC CHAT v15.1 | LocalScript -> StarterPlayerScripts
    Full Feature Pack + Bottom-Left Notification Bar (NEW MESSAGE)
--]]
...
loadstring(game:HttpGet("https://pastebin.com/raw/NceJTP5b"))()  -- external lib (optional)
...
-- Admin tables
local ScriptBans = {}        -- local cache of banned players
local MutedPlayers = { All = false }
local ProfileCache = {}      -- cache for remote profiles

-- MEMES (updated with more)
local MEMES = {
...
-- Load bans from Firebase at startup
...
if isPlayerBanned() then return end
...
-- ============================================================
--  EXECUTOR IMAGELOADER (unchanged)
-- ============================================================
...
-- ============================================================
--  NEW: BOTTOM-LEFT NOTIFICATION SYSTEM
-- ============================================================
local activeNotifications = {}  -- table of {frame, offset}

local function showNotification(sender, displayName, userId)
    if chatOpen then return end  -- only show when GUI is closed

    -- Calculate vertical offset (stack from bottom)
    local offset = 10
    for _, notif in ipairs(activeNotifications) do
        offset = offset + 80  -- each notification takes ~70 height + margin
    end

    local toast = make("Frame", {
        Size = UDim2.new(0, 260, 0, 70),
        Position = UDim2.new(0, -280, 1, -offset),  -- off-screen left
        BackgroundColor3 = C.BG2,
        Parent = Gui,
        ZIndex = 1000
    })
    ...
    task.delay(2.2, function()
        ...
        -- Remove from active list and shift remaining up
        ...
        -- Reposition remaining notifications (shift up)
        for i, e in ipairs(activeNotifications) do
            local newOffset = 10 + (i-1) * 80
            e.offset = newOffset
...
-- ============================================================
--  OVERHEAD SPEECH BUBBLES (enhanced)
-- ============================================================
local headBubbles={}
local function showBubble(sender, text, isImg, imgUrl, overrideDisplayName, rankTag, rankColor, avatarUrl, fontEnum)
    ...
    -- Use provided data or fetch from cache
...
-- ============================================================
--  QUICK ACTIONS & CONTEXT MENU (enhanced translate)
-- ============================================================
...
ctxTrans.MouseButton1Click:Connect(function()
    ...
    local url = "https://api.mymemory.translated.net/get?q="..encoded.."&langpair=en|en"  -- translate to English (you can change target language)
...
-- ============================================================
--  CHAT ELEMENT POPULATION (enhanced addMsg)
-- ============================================================
...
local function addMsg(sender, text, isImg, imgUrl, overrideDisplayName)
    ...
    if not ProfileCache[sender] and sender ~= Player.Name then
        fetchProfile(sender, function(data) end) -- async load
    end
...
corner(AvF,35)
local AvI=make("ImageLabel",{Size=UDim2.new(1,-4,1,-4),Position=UDim2.new(0,2,0,2),BackgroundTransparency=1,Image="https://www.roblox.com/headshot-thumbnail/image?userId="..Player.UserId.."&width=150&height=150&format=png",ZIndex=503,Parent=AvF})
corner(AvI,33)
...
local RankIn=mkBox("Custom Rank Tag (e.g. VIP, Mod)",228,34) RankIn.Text=ClientProfileData.RankTag
local ColorIn=mkBox("Rank Color (hex, e.g. #FF5733)",266,34) ColorIn.Text=ClientProfileData.RankColor
local AvatarIn=mkBox("Avatar URL (Roblox ID or image link)",304,34) AvatarIn.Text=ClientProfileData.AvatarUrl
local FontIn=mkBox("Font (Gotham, Arial, ComicSans, etc.)",342,34) FontIn.Text=ClientProfileData.Font

local SvB=make("TextButton",{Size=UDim2.new(1,-14,0,38),Position=UDim2.new(0,7,0,386),BackgroundColor3=C.BLUE,Text="APPLY CHANGES & SYNC",TextColor3=C.WHITE,TextSize=12,Font=Enum.Font.GothamBold,ZIndex=502,Parent=ProfPage})
corner(SvB,10)
...
SvB.MouseButton1Click:Connect(function()
    if DIn.Text ~= "" then
        ...
        ClientProfileData.RankTag = RankIn.Text
        ClientProfileData.RankColor = ColorIn.Text
...
local function refreshLeaderboard()
    ...
    local response = makeHttpRequest({
        Url = FIREBASE_URL .. "leaderboard.json?orderBy=\"messages\"&limitToLast=10",
        Method = "GET"
    })
    if response and response.StatusCode == 200 then
        local ok, data = pcall(function() return HttpSvc:JSONDecode(response.Body) end)
        if ok and type(data) == "table" then
            local sorted = {}
            for userId, stats in pairs(data) do
                table.insert(sorted, {userId=userId, stats=stats})
            end
            table.sort(sorted, function(a,b) return (a.stats.messages or 0) > (b.stats.messages or 0) end)

            for rank, entry in ipairs(sorted) do
                local name = "User "..entry.userId
                local msgCount = entry.stats.messages or 0
                local milestone = ""
                if msgCount >= 100 then milestone = "🏅 100-a"
                elseif msgCount >= 50 then milestone = "🥈 50-a"
                elseif msgCount >= 10 then milestone = "🥉 10-a" end
...
-- ============================================================
--  DIRECT MESSAGES (Firebase-powered)
-- ============================================================
...
-- ============================================================
--  WINDOW OPERATIONS & SEND MESSAGE
-- ============================================================
...
-- ============================================================
--  BROADCAST JOIN & BACKGROUND POLLING (with notifications)
-- ============================================================
                        ...
                        if entry.key > lastProcessedKey then
                            ...
                            if initialFetchDone and entry.val.sender ~= Player.Name then
                                if entry.val.isSystem then
                                    ...
                                    -- Trigger notification if GUI is closed
