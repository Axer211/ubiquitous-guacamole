--[[
    ...
    MODIFIED: Permanent messages (no auto-delete) + per-user cooldowns (text 2s, sticker 10s) with countdown display
--]]
...
local cooldowns = { text = {}, sticker = {} }
local activeCooldownType = nil
local countdownLabel = nil

local function getCooldownRemaining(type, playerName)
    local last = cooldowns[type][playerName]
    if not last then return 0 end
    local elapsed = os.time() - last
    local duration = (type == "text") and TEXT_COOLDOWN or STICKER_COOLDOWN
    local remaining = duration - elapsed
    if remaining <= 0 then
        cooldowns[type][playerName] = nil
        return 0
    end
    return remaining
end

local function setCooldown(type, playerName)
    cooldowns[type][playerName] = os.time()
    activeCooldownType = type
end

local function updateCountdownLabel()
    if not countdownLabel then return end
    if activeCooldownType then
        local remaining = getCooldownRemaining(activeCooldownType, Player.Name)
        if remaining > 0 then
            countdownLabel.Text = string.format("%ds", remaining)
            countdownLabel.Visible = true
        else
            activeCooldownType = nil
...
-- Cooldown label placed next to send button
countdownLabel = make("TextLabel",{
...
local function addMsg(sender, text, isImg, imgUrl, overrideDisplayName)
    ...
    -- Auto‑removal removed: messages stay permanently
...
function appendToMemeGalleryGrid(url, label)
    ...
    cell.MouseButton1Click:Connect(function()
        -- Check sticker cooldown before sending
        local remaining = getCooldownRemaining("sticker", Player.Name)
        if remaining > 0 then
        ...
        setCooldown("sticker", Player.Name)
        updateCountdownLabel()
        switchTab("SERVER")
    end)
end
...
local function sendMsg()
    ...
    -- Commands are not subject to cooldown
    if txt=="/clear" then
    ...
    local remaining = getCooldownRemaining("text", Player.Name)
    if remaining > 0 then
        -- cooldown active, do not send
        return
    end
