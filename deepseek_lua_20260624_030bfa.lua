--[[
    ...
    FIXES:
      ...
      - OWN MESSAGES NOW ALWAYS SHOW (mute check skips local player)
--]]
...
-- MEMES (full list – paste your original here)
local MEMES = {
    ...
    -- ... paste all your remaining meme entries here (they are the same as in your original file)
}
...
local function addMsg(sender, text, isImg, imgUrl, overrideDisplayName)
    ...
    -- MUTE CHECK: skip only if mute is on AND sender is NOT the local player
    if MutedPlayers.All or (sender ~= Player.Name and MutedPlayers[sender]) then
        return
    end
    ...
    -- Use overrideDisplayName if provided (for received messages), else use local data
    local displayName = overrideDisplayName
    if not displayName then
        if isMe then
            displayName = ClientProfileData.DisplayName
            if displayName == "" then displayName = Player.DisplayName end
        else
            displayName = pObj and pObj.DisplayName or sender
        end
    end
    ...
    -- If not cached and not self, fetch in background
...
-- ============================================================
--  TOAST & NOTIFICATION (UI) - moved up by increasing offset to 100
-- ============================================================
...
-- ============================================================
--  BUBBLE TRIGGER BUTTON (minimizer)
-- ============================================================
...
-- ============================================================
--  BROADCAST PAYLOAD - FIXED (same-server only) + profile fields
-- ============================================================
...
local function sendMsg()
    ...
    -- Normal message – now always shown
    addMsg(Player.Name, txt, false, nil)
                    ...
                    for _, entry in ipairs(sortedPackets) do
                        if entry.key > lastProcessedKey then
                            ...
                            if initialFetchDone and entry.val.sender ~= Player.Name then
                                -- Update ProfileCache with received profile data
