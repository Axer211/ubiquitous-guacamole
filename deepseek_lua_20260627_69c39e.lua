--[[
    ...
    IMPROVED: Permanent messages (no auto-delete) + cooldowns (text 2s, sticker 10s) with local notifications
    FIXED: Spam glitch and profile update bug
    ADDED: Meme popup (floating grid above input) instead of gallery tab switch
--]]
...
--  COOLDOWN SYSTEM
-- ============================================================
local TEXT_COOLDOWN = 2
local STICKER_COOLDOWN = 10
local cooldowns = { text = 0, sticker = 0 }

local function canSend(type)
    local last = cooldowns[type] or 0
    local now = os.time()
    local cooldownTime = (type == "text") and TEXT_COOLDOWN or STICKER_COOLDOWN
    if now - last < cooldownTime then
        return false, cooldownTime - (now - last)
    end
    return true, 0
end

local function setCooldown(type)
    cooldowns[type] = os.time()
end
...
-- ===== MEME POPUP (floating above input) =====
local MemePopupVisible = false
local MemePopup = make("Frame",{
    Size = UDim2.new(0, 360, 0, 180),
    Position = UDim2.new(0.5, -180, 0, 8), -- will be positioned later
...
corner(MemePopup, 12)
stroke(MemePopup, Color3.fromRGB(70,70,90), 1.5)

local PopupScroll = make("ScrollingFrame",{
...
-- Function to add a meme cell to the popup
local function addMemeToPopup(url, label)
    local cell = make("ImageButton",{
    ...
    corner(cell, 8)
    stroke(cell, Color3.fromRGB(55,55,75))
    ...
    cell.MouseButton1Click:Connect(function()
        -- Check sticker cooldown
        local can, rem = canSend("sticker")
        if not can then
            addSys("Please wait...")
            return
        end
        ...
        setCooldown("sticker")
        -- Close popup
        MemePopup.Visible = false
        MemePopupVisible = false
    end)
end

-- Populate popup with all memes
for _, meme in ipairs(MEMES) do
    addMemeToPopup(meme.url, meme.label)
end

-- Also add saved custom memes to popup (they will be added later when loaded)
local function loadSavedCustomGalleryPopup()
    ...
    if raw and raw ~= "" then
        ...
        if ok and type(data) == "table" then
            for _, customUrl in ipairs(data) do
                addMemeToPopup(customUrl, "Saved Link 📥")
            end
        end
    end
end
loadSavedCustomGalleryPopup()

-- Also update popup when saving custom URLs (we'll override the save function to also add to popup)
local originalSaveCustomUrl = saveCustomUrlToGallery
saveCustomUrlToGallery = function(url)
    originalSaveCustomUrl(url)
    -- Also add to popup if not already present
    local found = false
    for _, child in ipairs(PopupScroll:GetChildren()) do
        if child:IsA("ImageButton") and child.Image == getImageAsset(url) then
            found = true
            break
        end
    end
    if not found then
        addMemeToPopup(url, "Saved Link 📥")
    end
end

-- Click outside to close popup
local function closeMemePopup()
    if MemePopupVisible then
        MemePopup.Visible = false
        MemePopupVisible = false
    end
end
...
UIS.InputBegan:Connect(function(inp)
    if inp.UserInputType == Enum.UserInputType.MouseButton1 or inp.UserInputType == Enum.UserInputType.Touch then
        if MemePopupVisible then
            ...
            local abs = MemePopup.AbsolutePosition
            local size = MemePopup.AbsoluteSize
...
-- ============================================================
--  CHAT MESSAGE ADDERS (with permanent retention)
-- ============================================================
...
local function addMsg(sender, text, isImg, imgUrl, overrideDisplayName)
    ...
    -- For local player, always use ClientProfileData to avoid stale cache after profile update
    local displayName = overrideDisplayName or (isMe and ClientProfileData.DisplayName or (pObj and pObj.DisplayName or sender))
    local rankTag = (isMe and ClientProfileData.RankTag) or (ProfileCache[sender] and ProfileCache[sender].rankTag) or ""
    local rankColor = (isMe and ClientProfileData.RankColor) or (ProfileCache[sender] and ProfileCache[sender].rankColor) or "#FFFFFF"
    local avatarUrl = (isMe and ClientProfileData.AvatarUrl) or (ProfileCache[sender] and ProfileCache[sender].avatarUrl) or ""
    local fontName = (isMe and ClientProfileData.Font) or (ProfileCache[sender] and ProfileCache[sender].font) or "Gotham"
    ...
    -- Wrap UI creation in pcall to prevent rendering errors from breaking the chat
    local success, err = pcall(function()
        ...
        -- Messages stay permanently (no auto-delete)
    ...
    if not success then
        warn("[Xdemic] Failed to render message: ", err)
        -- Fallback: show a system message indicating a render error, but still add a simple message
...
function showBubble(sender, text, isImg, imgUrl, overrideDisplayName, rankTag, rankColor, avatarUrl, fontEnum)
    ...
    -- ======= Auto‑removal: 10s for stickers, 2s for text (unchanged) =======
...
-- ============================================================
--  GALLERY PAGE (kept but not opened by the meme button)
-- ============================================================
...
function appendToMemeGalleryGrid(url, label)
    ...
    cell.MouseButton1Click:Connect(function()
        -- Check sticker cooldown
        local can, remaining = canSend("sticker")
        if not can then
            addSys("Please wait...")
            return
        end
...
-- MBtn now toggles the meme popup instead of switching to gallery
MBtn.MouseButton1Click:Connect(function()
    -- Close URL box if open
    if urlOpen then
        URLBox.Visible = false
        urlOpen = false
    end
    -- Toggle popup
    MemePopupVisible = not MemePopupVisible
    MemePopup.Visible = MemePopupVisible
    if MemePopupVisible then
        -- Position the popup above the input bar
        local inputPos = IBar.AbsolutePosition
        local inputSize = IBar.AbsoluteSize
        local popupWidth = 360
        local popupHeight = 180
        -- We'll position it relative to SrvPage
        local srvAbs = SrvPage.AbsolutePosition
        local x = (IBar.AbsolutePosition.X - srvAbs.X) + (IBar.AbsoluteSize.X - popupWidth) / 2
        local y = IBar.AbsolutePosition.Y - srvAbs.Y - popupHeight - 4
        MemePopup.Position = UDim2.new(0, x, 0, y)
        MemePopup.Size = UDim2.new(0, popupWidth, 0, popupHeight)
    end
end)
...
local function closeChat() chatOpen=false Win.Visible=false URLBox.Visible=false urlOpen=false QuickActionMenu.Visible=false MemePopup.Visible=false MemePopupVisible=false end
...
local function sendMsg()
    ...
    -- Commands are exempt from cooldown
    if txt=="/clear" then
    ...
    -- Normal text message: check text cooldown
    local can, remaining = canSend("text")
    if not can then
        addSys("Please wait...")
        return
    end
