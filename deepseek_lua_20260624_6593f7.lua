--[[
    ...
    FIXES:
      - Profile updates now broadcast full profile data (displayName, avatarUrl, rankTag, rankColor, font)
      - Meme bubbles now properly self‑destroy after 6 seconds
      - Profile cache is refreshed immediately after saving
      - Notification offset increased from 70 to 100 (moved up)
--]]
...
-- MEMES table (truncated for brevity – paste your full list here)
local MEMES = {
    {url="https://cdn.discordapp.com/...", label="genz kiddo🥀"},
    -- ... (add your other meme entries)
}
...
-- ============================================================
--  SCREEN GUI & MAIN WINDOW (unchanged)
-- ============================================================
...
-- The broadcast payload now includes full profile data
...
-- addMsg now uses passed profile data if available
local function addMsg(sender, text, isImg, imgUrl, overrideDisplayName, overrideAvatarUrl, overrideRankTag, overrideRankColor, overrideFont)
    ...
    -- Use override if provided, else fallback to cache or defaults
    ...
    local avatarUrl = overrideAvatarUrl or (isMe and ClientProfileData.AvatarUrl or (profile and profile.avatarUrl) or "")
    local rankTag = overrideRankTag or (isMe and ClientProfileData.RankTag or (profile and profile.rankTag) or "")
    local rankColor = overrideRankColor or (isMe and ClientProfileData.RankColor or (profile and profile.rankColor) or "#FFFFFF")
    local fontName = overrideFont or (isMe and ClientProfileData.Font or (profile and profile.font) or "Gotham")
    local fontEnum = Enum.Font[fontName] or Enum.Font.Gotham

    -- If no override and not cached, fetch asynchronously
    if not overrideDisplayName and not ProfileCache[sender] and sender ~= Player.Name then
...
function showBubble(sender, text, isImg, imgUrl, overrideDisplayName, rankTag, rankColor, avatarUrl, fontEnum)
    ...
    -- FIX: Meme bubbles now disappear after 6 seconds
...
-- ============================================================
--  QUICK ACTION MENU & CONTEXT MENU (unchanged)
-- ============================================================
...
-- ============================================================
--  TOAST & NOTIFICATION (UI) - moved up by increasing offset
-- ============================================================
...
local function showNotification(sender, displayName, userId)
    ...
    -- FIX: Increased base offset from 70 to 100 (moved up)
    local offset = 100
...
-- ============================================================
--  DIRECT MESSAGES PAGE (unchanged)
-- ============================================================
                        ...
                        if entry.key > lastProcessedKey then
                            ...
                            if initialFetchDone and entry.val.sender ~= Player.Name then
                                if entry.val.isSystem then
                                    ...
                                    -- Pass profile data from packet
                                    addMsg(
                                        entry.val.sender,
                                        entry.val.text,
                                        entry.val.isImg,
                                        entry.val.imgUrl,
                                        entry.val.displayName,
                                        entry.val.avatarUrl,
                                        entry.val.rankTag,
                                        entry.val.rankColor,
                                        entry.val.font
