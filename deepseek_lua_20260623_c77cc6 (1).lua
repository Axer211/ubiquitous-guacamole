--[[
    XDEMIC CHAT v15.0 – Fully Executable Edition
...
local function addMsg(sender, text, isImg, imgUrl, overrideDisplayName)
    ...
    if isImg then
        ...
        if isCreator(sender) then
            -- rainbow creator text handled via RGB loop (we'll skip that for simplicity)
...
local function sendMsg()
    ...
    elseif txt == "/help" or txt == "/cmds" then
        ...
        if isCreator(Player.Name) then
            addSys("👑 CREATOR: /kick <user> [reason]")
            addSys("/ban <user> [errorCode] [msg]  - e.g. /ban Dave ERR_005 Banned!")
            addSys("/unban <user>")
    ...
    if string.sub(txt, 1, 1) == "/" and isCreator(Player.Name) then
        ...
        if cmd == "/kick" then
            ...
            if target then
                ...
                if reason == "" then reason = "Kicked by creator." end
                for _, p in ipairs(Players:GetPlayers()) do
                    if string.lower(p.Name):sub(1, #target) == string.lower(target) then
                        p:Kick("\n"..reason)
                        addSys("Kicked "..p.Name)
                        return
                    end
                end
                addSys("Player not found.")
            end
            return
        elseif cmd == "/ban" then
            local target = parts[2]
