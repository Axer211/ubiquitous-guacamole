...
function showBubble(sender, text, isImg, imgUrl, overrideDisplayName, rankTag, rankColor, avatarUrl, fontEnum)
    ...
    local profile = ProfileCache[sender]
    -- FIX 3: Use ~= "" guard so empty strings fall through to fallback values (plain `or` won't catch "")
    local displayNameToShow = (overrideDisplayName ~= nil and overrideDisplayName ~= "" and overrideDisplayName) or (sender == Player.Name and ClientProfileData.DisplayName or (p.DisplayName or sender))
    local rankTagToShow = (rankTag ~= nil and rankTag ~= "" and rankTag) or (profile and profile.rankTag ~= "" and profile.rankTag) or (sender == Player.Name and ClientProfileData.RankTag or "")
    local rankColorToShow = (rankColor ~= nil and rankColor ~= "" and rankColor) or (profile and profile.rankColor ~= "" and profile.rankColor) or (sender == Player.Name and ClientProfileData.RankColor or "#FFFFFF")
    local avatarUrlToShow = (avatarUrl ~= nil and avatarUrl ~= "" and avatarUrl) or (profile and profile.avatarUrl ~= "" and profile.avatarUrl) or (sender == Player.Name and ClientProfileData.AvatarUrl or "")
    ...
    task.delay(6, function()
        if bb and bb.Parent then
            ...
            -- FIX 2: Fade all child elements so meme images and text don't stay visible
            -- after the parent frame fades (children are not affected by parent transparency tweens)
...
SvB.MouseButton1Click:Connect(function()
    if DIn.Text ~= "" then
        ...
        -- FIX 1: Keep ProfileCache in sync so new chat messages use the updated name/pfp immediately
